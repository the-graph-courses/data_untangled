...
```{r, eval = F, echo = FALSE}
# Pour cette première question pratique, nous fournirons la réponse.
# Exécutez ce bloc, puis exécutez les fonctions CHECK et HINT dans les blocs de code plus bas
...
```{r, eval = F, echo = FALSE}
Q_drop_resp_under_20 <- "VOTRE RÉPONSE ICI" 
```
:::
...
```{r eval = F}
yaounde %>% filter(respiration_frequency >= 20 | is.na(respiration_frequency))
```
