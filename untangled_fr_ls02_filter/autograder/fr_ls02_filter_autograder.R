...
.CHECK_Q_is_pregnant <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_is_pregnant)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_is_pregnant, .Q_is_pregnant))) return(c(value = 1, message = paste("Correct!")))
        
        return(c(value = 0, message = "Incorrect. Veuillez réessayer."))
      }
    .apply_autograder()
  }
...
.HINT_Q_is_pregnant <- function(){ 
  'INDICE: 
  VOSDONNEES %>% filter(CONDITION)' -> out
  cat(out)
}
...
.CHECK_Q_female_nrow <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.numeric(Q_female_nrow)) return(c(value = -1, message = "Le résultat doit être numérique."))
        
        if (identical(Q_female_nrow, .Q_female_nrow)) return(c(value = 1, message = paste("Correct!")))
...
.CHECK_Q_under_18 <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_under_18)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_under_18, .Q_under_18))) return(c(value = 1, message = paste("Correct!")))
...
.CHECK_Q_tsinga_messa <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_tsinga_messa)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_tsinga_messa, .Q_tsinga_messa))) return(c(value = 1, message = paste("Correct!")))
...
.CHECK_Q_male_positive <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_male_positive)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_male_positive, .Q_male_positive))) return(c(value = 1, message = paste("Correct!")))
...
.CHECK_Q_child_primary <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_child_primary)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_child_primary, .Q_child_primary_wrong_operator))) return(c(value = 0, message = paste("Incorrect. Vous devriez utiliser l'opérateur `|`.",
                                                                                                                     "En réalité, nous voulons des enfants OU toute personne ayant une éducation supérieure de type 'Primary'",
                                                                                                                     "(bien que la question mentionne 'et'). Réfléchissez-y un moment.",
                                                                                                                     "C'est assez contre-intuitif !")))
        
        if (isTRUE(all.equal(Q_child_primary, .Q_child_primary))) return(c(value = 1, message = paste("Correct!",
                                                                                                      "En réalité, nous voulons des enfants OU toute personne ayant une éducation supérieure 'Primaire'",
                                                                                                      "(bien que la question dise 'et').",
                                                                                                      "C'est assez contre-intuitif !")))
...
.HINT_Q_child_primary <- function(){ 
  'INDICE: 
  VOSDONNEES %>% filter(CONDITION1 | CONDITION2)' -> out
  cat(out)
}
...
.CHECK_Q_not_tsinga_messa <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_not_tsinga_messa)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_not_tsinga_messa, .Q_not_tsinga_messa))) return(c(value = 1, message = paste("Correct!")))
...
.CHECK_Q_na_smoker <-
  function() {
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_na_smoker)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
        
        if (isTRUE(all.equal(Q_na_smoker, .Q_na_smoker))) return(c(value = 1, message = paste("Correct!")))
    ...
    .autograder <<- 
      function(){
        if (!is.data.frame(Q_drop_resp_under_20))
          return(c(value = -1, message = "Incorrect. Le résultat devrait être un dataframe."))
        if (isTRUE(all.equal(Q_drop_resp_under_20, .Q_drop_resp_under_20_wrong_forgot_na))) 
          return(c(value = 0, message = "Incorrect. N'oubliez pas de prendre également en compte ceux avec des valeurs manquantes (NA)."))
        
        if (isTRUE(all.equal(Q_drop_resp_under_20, .Q_drop_resp_under_20))) 
          return(c(value = 1, message = paste("Correct!")))
...
.HINT_Q_drop_resp_under_20 <- function(){ 
  'INDICE: 
  VOSDONNEES %>% filter(CONDITION1 | NA_CONDITION )' -> out
  cat(out)
}
...
#     .autograder <<- 
#       function(){
#         if (!is.data.frame(Q_is_pregnant0)) return(c(value = -1, message = "Le résultat devrait être un dataframe."))
#         
#         if (isTRUE(all.equal(Q_is_pregnant0, .Q_is_pregnant0))) return(c(value = 1, message = paste("Correct!")))
#         
#         return(c(value = 0, message = "Incorrect. Veuillez réessayer."))
#       }
#     .apply_autograder()
...
# 
# .HINT_Q_10 <- function(){ 
#   'VOSDONNEES %>% filter(row_number() %in% SELECTION)' -> out
#   cat(out)
# }
# 
# 
