...
fr_upload_and_link <-
  function(x,
    ...
    if (which %in% c("both", "download")) 
      cat(paste0("Clickez [**ici**]", "(", gdrive_link, "){target='_blank'}", " pour consulter et télécharger les données."))
    
    if (which %in% c("both", "read")) {
      cat(paste0(" Vous pouvez également l'importer directement dans R à l'aide du code ci-dessous :\n")) ## phrasing here is stupid for the "read" argument value. Should remove the "or". 
      cat(gdrive_import_chunk(gdrive_link, filename, format))
    }
