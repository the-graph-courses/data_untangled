...
::: watch-out
-   `TRUE` should be the *final* condition in `case_when()`.

-   If it is first condition, it will take precedence over all others
...
-   All individuals are now coded with "Not child"

-   Because the `TRUE` condition was placed first.

-   More on the issue of precedence further below.
:::
...
::: practice
As before, using the `flu_linelist` data, make a new column, called `age_group`, that has the value "Below 60" for people under 60 and "60 and above" for people aged 60 and up.

**But this time, also set those with missing ages to "Missing age".**
...
yaounde_educ %>%
  ...
  mutate(highest_educ_recode = 
           case_when(highest_education %in% c("University", "Doctorate") ~ "Post-secondary"))
```
...
yaounde_age_sex %>%
  mutate(recruit = 
           case_when(age_years >= 20 & age_years <= 29 & sex=="Female" ~ "Recruit to female study",
