...
# autograders
mute(here("ch04_data_wrangling/lessons/ls01_select_rename_autograder.R"))
```
...
-   You can keep or drop columns using the `select()` function from the {dplyr} package.
...
-   COVID-19 serological survey conducted in Yaounde, Cameroon in late 2020.

-   How many people have been infected with COVID-19 in the region.
...
```{r}
yaounde %>% select(contains("_drug_")) # Columns that contain an underscore
```
