...
La colonne `gender` de l'ensemble de données `flu_linelist` contient les valeurs "f", "m" et `NA` :
...
En utilisant les données `liste_influ`, modifiez la colonne existante `outcome` en remplaçant la valeur "Recover" par "Recovery".
...
Voici une façon (pas la seule façon) de répondre à la deuxième question de cette exercice:
