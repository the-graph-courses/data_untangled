...
Out of the entire sample of individuals in the `flu_linelist` dataset, what percentage are below 60? (Repeat the above procedure but with the `60` cutoff, then call `tabyl()` on the age group variable.)
...
Recall that following BMI categories:

-   A healthy BMI is a weight between 18.5 and 25: the person has a normal weight.

-   If the BMI is inferior to 18.5, the person is considered underweight.

-   If the BMI is between 25 and 30, the person is considered overweight.

-   If the BMI is above 30, the person is considered obese.
