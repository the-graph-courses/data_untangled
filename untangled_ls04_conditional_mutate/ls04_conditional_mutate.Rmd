...
What percentage of individuals in the dataset are below 60? (You can use the `tabyl()` function to compute this.)
...
This `TRUE` condition can be read as "for everything else...". So the full `case_when()` statement used above, `age_years < 18 ~ "Child", TRUE ~ "Not child"`, would then be read as: "if age is below 18, input 'Child' and *for everyone else not yet matched*, input 'Not child'".
...
Rather than match missing values with `TRUE` as we did above, we can match them manually with `is.na()`. Below we match `NA` ages with `is.na()` and set their age group to "Missing age":
...
But first, we will inspire ourselves from what we learnt in the `mutate()` lesson and recreate the BMI variable. This involves first converting the `height_cm` variable to meters. Then using the BMI's formula we compute the BMI with the `weight_kg` and `height_m` variables.
...
Now if we want to use the BMI as a health indicator, we need to think about what the BMI values represent.
...
But you can see that the levels of BMI are defined in alphabetical order from Normal weight to Too thin, instead of from lightest (Too thin) to heaviest (Obese). Remember that if you want to have a certain order you can make `BMI_classification` a factor using `mutate()` and define its levels.
...
yaounde_BMI %>%
  mutate(BMI_classification = case_when(BMI < 18.5 ~'Too thin',
                                        # the variable name was forgotten
                                        BMI >= 18.5 & <= 25 ~ 'Normal weight',
                                         # the variable name was included
                                        BMI > 25 & BMI <= 30 ~ 'Overweight',
                                        BMI > 30 ~ 'Obese'))
```
...
Q_adolescent_grouping <- 
  flu_linelist %>% 
  mutate(adolescent = ______________________________)
```
...
This `case_when()` statement can be read in simple terms as 'If the person has anosmia/ageusia, input "Probable COVID", and otherwise, if the person has cough AND fever, input "Possible COVID"'.
