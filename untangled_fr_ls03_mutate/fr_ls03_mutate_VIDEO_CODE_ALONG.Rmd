...
Le dataframe `sarcopenia` contient le poids en kilogrammes. Créez `q_weight_to_grams` pour le poids en grammes (1 kg est égale à 1000 grammes).
...
Avec un sous-ensemble féminin du jeu de données `sarcopenia`, ajoutez une variable `low_grip_strength` qui est `TRUE` pour les femmes ayant une force de préhension \< 20 kg et `FALSE` pour les autres.

```{r}
# Complétez le code avec votre réponse :
q_women_low_grip_strength <- 
  sarcopenia %>% 
  filter(sex_male_1_female_0 == 0) %>% # d'abord, nous filtrons le jeu de données pour les femmes uniquement
  mutate(________________________)
```

**Pratique 3_2**

Quel pourcentage de femmes interrogées ont une faible force de préhension selon la définition ci-dessus ? Entrez votre réponse en tant que nombre sans guillemets (ex. : 43.3 ou 12.2).

```{r, eval = FALSE}
q_prop_women_low_grip_strength <- VOTRE_RÉPONSE_ICI
```
:::
...
q_age_integer <- 
  sarcopenia %>% 
  mutate(age_integer = ________________)
```
:::
