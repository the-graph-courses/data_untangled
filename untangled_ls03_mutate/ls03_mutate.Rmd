...
![](images/mutate_equivalent_in_excel.gif){width="321"}
:::
...
Hopefully you now see that the mutate function is quite user-friendly. In theory, we could end the lesson here, because you now know how to use `mutate()` 😃. But of course, the devil will be in the details---the interesting thing is not `muxtate()` itself but what goes *inside* the `mutate()` call.
...
It is sometimes useful to rank rows based on the value of a column. For example, you may want to rank survey respondents in order of their height. You can do this with `mutate()` and the `rank()` function.

```{r}
yao_height %>% 
  mutate(height_rank = rank(height_cm, ties.method = "min"))
```

How should we understand this output? It shows, for example, that the first respondent, with a height of 169 cm, is the 659th shortest person.

The `ties.method` argument specifies how individuals with the same height should be ranked. `ties.method = "min"` means that the lower (minimum) rank should be used. What this implies should become clear when you `arrange()` the data frame by rank:
...
As you can see, the respondents in row 5 and 6 are both 99cm tall. And because of the `ties.method = "min"` argument, they are both ranked the 5th tallest, because 5 is the lower rank between 5 and 6.

------------------------------------------------------------------------

If you want to reorder the rank, such that the tallest person is ranked number 1, you can wrap the variable to be ranked in the `desc()` function, which stands for "descending":

```{r}
yao_height %>% 
  mutate(height_rank = rank(desc(height_cm), ties.method = "min")) %>% 
  arrange(height_rank)
```

There are some very tall people!

::: practice
The `grip_strength_kg` variable in the `sarcopenia` data frame reports participant hand grip strength in kilograms as measured by a hand dynamometer. Add a variable called `grip_strength_rank` , which ranks participants by their grip strength. The person with the highest grip strength should be number 1. Use the "min" ties method.

```{r, eval = FALSE}
# Complete the code with your answer:
Q_sarcopenia_grip_strength <- 
  sarcopenia %>% 
  mutate(grip_strength_rank = ______________________________________)
```
...
# Each question has a solution function like this. 
# (Replace HINT with SOLUTION in the function name)
# But you will need to type this out on your own, 
# (to discourage you from looking at the solution before answering the question)
```
...
$$ASM(kg)= (0.244 \times weight(kg)) + (7.8 \times height(meters)) + (6.6 \times sex) - (0.098 \times age) - 4.5$$
...
But your data wrangling journey isn't over yet! In our next lessons, we will learn how to create complex data summaries and how to create and work with data frame groups. Intrigued? See you next lesson.
