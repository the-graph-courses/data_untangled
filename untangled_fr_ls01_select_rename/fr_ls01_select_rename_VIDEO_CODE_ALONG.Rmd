...
Q_weight_height <- "VOTRE RÉPONSE"

# Les solutions pour toutes les questions se trouvent au bas de ce document.
```
...
Voici les solutions pour tous les exercices pratiques :

## Question 1 : Sélectionner poids et taille

```{r, eval = F}
Q_weight_height <- yaounde %>% select(weight_kg, height_cm)
```

## Question 2 : Sélectionner les colonnes 16 et 22

```{r, eval = F}
Q_cols_16_22 <- yaounde %>% select(16, 22)
```

## Question 3 : Sélectionner de symptoms à sequelae

```{r, eval = F}
Q_symp_to_sequel <- yaounde %>% select(symptoms:sequelae)
```

## Question 4 : Retirer de highest_education à consultation

```{r, eval = F}
Q_educ_consult <- yaounde %>% select(!highest_education:consultation)
```

## Question 5 : Colonnes commençant par "is_"

```{r, eval = F}
Q_starts_with_is <- yaounde %>% select(starts_with("is_"))
```

## Question 6 : Déplacer les colonnes "is_" au début
